<template>
  <div class="list-container relative text-base group overflow-hidden">
    <div class="flex items-center gap-2 ml-1 mt-1">
      <button class="w-5 h-5 bg-gray-ultra-light flex items-center justify-center z-40">
        <IconKeyboardUp />
      </button>
      <a href="/" class="text-base font-bold text-gray-dark flex gap-2 items-center">
        {{ department.name }} ({{ department.jobs.length }})
        <span class="hidden group-hover:block"><IconArrowRight /></span>
      </a>
    </div>
    <ul class="flex flex-col text-sm gap-3 ml-10 my-2">
...
const showSeeMoreButton = computed(() => {
  return props.department.jobs.length > 5 && !showAllJobs.value
})
...
<style lang="scss" scoped>
  li::before {
    @apply content-[''] absolute bottom-[8px] left-[-28px] w-5 h-0.5 bg-gray-ultra-light z-10;
  }

  li > a::before {
    @apply content-[''] inline-block w-2 h-2 bg-green-600 rounded-full;
  }

  .list-container::before {
    @apply content-[''] absolute top-4 left-3 h-full w-0.5 bg-gray-ultra-light text-gray-ultra-light;
  }
</style>
